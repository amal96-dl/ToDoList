<template>
  <div class="todo px-6">
    ...
    <v-row class="text-center"> 
      <v-col class="mb-4">
        <h1 class="display-2 font-weight-bold mb-3 blue--text">
          Todo list
        </h1>
      </v-col>
    </v-row>
    <v-row class="text-center">
      <v-col class="mb-4">
        ...
        <v-row class="my-1" align="center">
          ...
          <v-spacer></v-spacer>
          <v-progress-circular v-model="progress" class="me-2" color="green">
          </v-progress-circular>
        </v-row>
        <v-divider class="mb-4"></v-divider>
      </v-col>
    </v-row>
    <v-text-field outlined label="Add task" clearable hide-details v-model="newTaskTitle" @click:append="addTask" @keyup.enter="addTask" append-icon="mdi-plus-circle" class="mb-4"></v-text-field>
      ...
      <v-snackbar v-model="snackbar.active">
        ...
        <template v-slot:action="{ attrs }">
          <v-btn color="pink" text v-bind="attrs" @click="snackbar.active = false">
            Close 
          </v-btn>
        </template>
      </v-snackbar>
